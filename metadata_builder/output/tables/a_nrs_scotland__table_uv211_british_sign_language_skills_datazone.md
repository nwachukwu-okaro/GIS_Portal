# Table Uv211 British Sign Language Skills Datazone

## Overview

- **Identifier:** `a_nrs_scotland/table_uv211_british_sign_language_skills_datazone`
- **Source organisation:** National Records of Scotland
- **Source:** https://www.nrscotland.gov.uk/statistics-and-data/geography/our-products
- **Geographic coverage:** Scotland
- **Schema:** `a_nrs_scotland`
- **Table:** `table_uv211_british_sign_language_skills_datazone`
- **Geometry:** Non-spatial
- **CRS:** Not applicable or unknown
- **Rows:** 7392
- **Columns:** 4
- **Metadata status:** source_mapped

## Description

Table Uv211 British Sign Language Skills Datazone is an authoritative dataset published by National Records of Scotland. It contains records relating to table uv211 british sign language skills datazone.

## Columns

| Column | Data type | Meaning | Semantic role | Filter | Search | Join |
|---|---|---|---|---|---|---|
| `geography_code` | `text` | Code assigned by the source dataset. | code | Yes | No | No |
| `all_people_aged_3_and_over` | `double precision` | Count or numeric value for all people aged 3 and over in the represented area. | statistical_value | Yes | No | No |
| `bsl_user` | `double precision` | Count or numeric value for bsl user in the represented area. | statistical_value | Yes | No | No |
| `not_a_bsl_user` | `double precision` | Count or numeric value for not a bsl user in the represented area. | statistical_value | Yes | No | No |

## Supported operations

- filter
- select
- attribute_join
- export

## Metadata warnings

- Licence has not yet been verified.

## Provenance

Technical facts were extracted from PostGIS. Publisher information was inherited from the curated schema source registry.
